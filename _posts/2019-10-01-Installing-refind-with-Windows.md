...
Microsoft's folder intuitively contains files used to boot Windows. I'll resume what I think is important about the boot process, but if you want to know more, I suggest you start [here](https://uefi.org/sites/default/files/resources/UEFI-Plugfest-WindowsBootEnvironment.pdf) (this link is where I'm getting all the info below).
...
One more thing, add the following entry to your `refind.conf` file, replacing `{folder}` to anything **BUT** `Microsoft`, I'll use `_Microsoft`:
